...
function submit(delete_account) {
    ...
    if (delete_account) {
        router.delete(base_url + 'delete_account', form)
    }
    else {
...
<template>
    <Head title="HF / edit profile" />

    <Layout page="Edit Profile" :user="user">
        
    <form @submit.prevent="submit(false)">
        <div class="mx-auto mt-8 sm:mt-16 w-full sm:w-2/3 lg:w-1/3">
            <div class="mb-3">
                Name:
                <br>
            ...
            <div class="mb-3">
                New Password <span class="text-sm">(leave blank unless you are changing your password)</span>:
                <br>
            ...
            <button class="border-2 border-black rounded p-1 mt-5 text-black text-base sm:text-2xl" type="submit">Update Profile</button>
            <button type="button" class="border-2 border-red-500 rounded p-1 mt-5 ml-5 text-red-500 text-base sm:text-2xl" @click="submit(true)">Delete Account</button>
